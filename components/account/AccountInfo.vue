      ...
      <div class="lg:grid lg:grid-cols-11 lg:gap-60">
        ...
        <div class="lg:col-span-7">
          <Avatar
            ...
            :loading="loading"
            :src='"https://images.staymenity.com/img/default/user.png"'
          >
            <div class="font-semibold">{{ userData.first_name }} {{ userData.last_name }}</div>
            <span class="text-gray-900">Joined in {{ new Date(getFullYear).getFullYear() }}</span>
          </Avatar>
            <!-- :src="userData.image" -->
          <div v-if="!isMobile" class="flex items-start justify-between mb-45">
            <div>
              ...
              <div class="text-gray">Joined in {{ new Date(getFullYear).getFullYear() }}</div>
            </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      // },
      getFullYear() {
        return this.$auth?.user.registered_at
      },
      userData() {
        if (this.type === 'current') {
          // console.log('this.users', this.$auth?.user)
    ...
    methods: {
      ...
      fetchReviews() {
        ...
        const url = 'https://api.staymenity.com/public/api/reviews'
        // if (this.$auth.user.current_role === 'host') {
        //   url = `/host/${this.userData.id}/reviews`
        // }
        // if (this.$auth.user.current_role === 'guest') {
